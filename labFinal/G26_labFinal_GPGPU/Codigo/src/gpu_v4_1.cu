...
__global__ void filtro_mediana_kernel_v4_1(uchar* d_input, uchar* d_output, int width, int height, int W, float threshold) {
    ...
    // buckets en shared
    uchar* buckets = &sharedMem[blockDim.x * blockDim.y + 256 * sharedIndex];

    // inicializar buckets en 0
    int i; // se declara fuera del for para usarla luego
...
void filtro_mediana_gpu_v4_1(uchar* img_in, uchar* img_out, int width, int height, int W, int blockSize) {
    ...
    filtro_mediana_kernel_v4_1<<<gridDim, blockDim, sharedMemSize>>>(d_in, d_out, width, height, W, 0);
