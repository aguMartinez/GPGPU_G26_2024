...
__global__ void decrypt_kernel_ej3B(int *d_M, int * d_H, int * d_MH, dim3 numBlocks)
{
	...
	int block_pos = blockIdx.y * numBlocks.x + blockIdx.x;
...
__global__ void reduction(float * output, float * input){
	__shared__ float intermedio[HISTOGRAM_LENGTH];
...
int main(int argc, char *argv[])
{
	...
	decrypt_kernel_ej3B<<<numBlocks, threadsPerBlock>>>(d_M, d_H, d_MH, numBlocks);
