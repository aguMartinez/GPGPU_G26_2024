...
__global__ void decrypt_kernel_ej1A(int *d_message, int length)
{
	int index = threadIdx.x;
	if(index < length){
		d_message[index] = modulo(A_MMI_M*(d_message[index] - B), M);
	}
}

__global__ void decrypt_kernel_ej1B(int *d_message, int length)
{
...
__global__ void decrypt_kernel_ej1C(int *d_message, int length)
{
	int index = blockDim.x * blockIdx.x + threadIdx.x;
...
// Se utiliza un solo bloque de threads
void ej1A(int* d_message, int length) {
	int threads_per_block = 256;
	int blocks_per_grid = 1;

	decrypt_kernel_ej1A<<<blocks_per_grid, threads_per_block>>>(d_message, length);
}

// En esta parte se modifica la definicion de grilla para que utilice
// bloques suficientes para procesar el texto.
void ej1B(int* d_message, int length) {
	int threads_per_block = 256;
	int blocks_per_grid = (length + threads_per_block - 1) / threads_per_block;

	decrypt_kernel_ej1B<<<blocks_per_grid, threads_per_block>>>(d_message, length);
}

// Se modifica la definicion de grilla para utilizar 128 bloques 
// Se puede modificar blocks_per_grid para definir una cantidad arbritaria de bloques.
void ej1C(int* d_message, int length) {
	int blocks_per_grid = 128;
	int threads_per_block = 256;

	decrypt_kernel_ej1C<<<blocks_per_grid, threads_per_block>>>(d_message, length);
}
