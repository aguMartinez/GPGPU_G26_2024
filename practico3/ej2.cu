...
__global__ void sum4thPosition_kernel(int* d_M, int* d_MRes){
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y

    if (x < n && y < n && x+4 < n){
        d_MRes[y + x * n] += d_M[4*x + y * n]; //probablemente con atomic add
    }
}
 ...
 int main(){
    ...
    sum4thPosition_kernel<<<blocks_per_grid, threads_per_block>>>(d_M, size);
