...
struct IncrementIndex {
    ...
    void operator()(int i) {
        ...
        int vect_size = (nnz_row == 0) ? 6 : (nnz_row == 1) ? 0 : (nnz_row <= 2) ? 1 : (nnz_row <= 4) ? 2 : (nnz_row <= 8) ? 3 : (nnz_row <= 16) ? 4 : 5;
...
struct IndexAndSizeCalculator {
    ...
    IndexAndSizeCalculator(const int* niveles, const int* RowPtrL_h) 
        : niveles(niveles), RowPtrL_h(RowPtrL_h) {}

    __device__
    thrust::tuple<int, int> operator()(int i) const {
        ...
        return thrust::make_tuple(7 * idepth + vect_size, i);
    }
};
//FIN STRUCTS DE 3.
...
int ordenar_filas( int* RowPtrL, int* ColIdxL, VALUE_TYPE * Val, int n, int* iorder){
    ...
    thrust::for_each(thrust::device, thrust::counting_iterator<int>(0), thrust::counting_iterator<int>(n), IncrementIndex{thrust::raw_pointer_cast(d_ivects_T.data()), thrust::raw_pointer_cast(d_niveles_T.data()), thrust::raw_pointer_cast(d_RowPtrL_T.data())});
    //END Paralelizar Conteo

    //lo normal sería copiar el resultado al host, pero no es necesario porque se utiliza inmediatmente después...
    //thrust::copy(output.begin(), output.end(), ivects); 
    ...
    //thrust::device_vector<int> d_ivects(ivects, ivects + length);
    ...
    //BEGIN Paralelizar
    /*
    //thrust::device_vector<int> d_niveles_T(niveles, niveles + n);
    //thrust::device_vector<int> d_RowPtrL_T(RowPtrL_h, RowPtrL_h + n + 1);
    
    thrust::device_vector<int> d_iorder_T(n);

    // Vector to hold the output of transform
    thrust::device_vector<thrust::tuple<int, int>> indices(n);

    // Calculate indices and sizes
    thrust::transform(thrust::device, thrust::counting_iterator<int>(0), thrust::counting_iterator<int>(n), indices.begin(), IndexAndSizeCalculator(thrust::raw_pointer_cast(d_niveles_T.data()), thrust::raw_pointer_cast(d_RowPtrL_T.data())));

    // Scatter to place each index in its new position
    thrust::scatter(thrust::device, thrust::counting_iterator<int>(0), thrust::counting_iterator<int>(n), thrust::make_transform_iterator(indices.begin(), thrust::get<1>()), d_iorder_T.begin());
