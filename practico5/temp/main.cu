...
//Operación de Paralelizar 3:
/*
struct calculate_vect_size{
    int* keys;
    int* value;
    const int* ivects;
    const int* niveles;
    ...
    int vect_size = 
        ...
        keys_ivects[i] = ivects[7 * idepth + vect_size];
        keys_i[i] = i;
    }
};
*/
/*
struct IndexAndSizeCalculator{
    int* ivects;
    int* ivect_size;
    int* iorder;
    const int* niveles;
    ...
    IndexAndSizeCalculator(int* _ivects, int* _ivect_size, int* _iorder, const int* _niveles, const int* _RowPtrL_h) 
        : ivects(_ivects), ivect_size(_ivect_size), iorder(_iorder), niveles(_niveles), RowPtrL_h(_RowPtrL_h) {}
    ...
    void operator()(int i) {
        ...
        //TODO: PROBLEMA ACÁ, CONDICIONES DE CARRERA... PROBABLEMENTE HAYA QUE HACERLAS POR SEPARADO...
        int index = 7 * idepth + vect_size;

        iorder[ ivects[index] ] = i;  
        ivect_size[ ivects[index] ] = (vect_size == 6) ? 0 : pow(2, vect_size);
        atomicAdd(&ivects[index], 1);
    }
    */
...
__device__ int calculate_vect_size(int nnz_row) {
    if (nnz_row == 0)
        return 6;
    else if (nnz_row == 1)
        return 0;
...
int ordenar_filas( int* RowPtrL, int* ColIdxL, VALUE_TYPE * Val, int n, int* iorder){
    ...
    thrust::copy(d_ivects_T.begin(), d_ivects_T.end(), ivects);
    ...
    */
    ...
    thrust::transform(
        ...
        keys.begin(),
        [=] __device__ (int i) {
            int idepth = d_niveles_T[i] - 1;
            int vect_size = calculate_vect_size(d_RowPtrL_T[i+1] - d_RowPtrL_T[i] - 1);
            return 7 * idepth + vect_size;
        });

    int max_key = thrust::reduce(thrust::device, keys.begin(), keys.end(), 0, thrust::maximum<int>());

    thrust::device_vector<int> offsets(max_key + 1, 0);

    // Ordenar keys para prepararlas para reduce_by_key
    thrust::sort(thrust::device, keys.begin(), keys.end());

    thrust::device_vector<int> counts(max_key + 1, 0);

    thrust::device_vector<int> unique_keys(max_key + 1);

    auto end = thrust::reduce_by_key(thrust::device, keys.begin(), keys.end(), thrust::constant_iterator<int>(1), unique_keys.begin(), counts.begin()).second;
    ...
    */
    ...
    [=] __device__ (int i) {
        int idepth = niveles[i] - 1;
        int vect_size = calculate_vect_size(RowPtrL_h[i+1] - RowPtrL_h[i] - 1); // Función para determinar vect_size

        // Clave compuesta por nivel y vect_size
        int key = 7 * idepth + vect_size;

        // Asignación atómica a iorder usando el offset calculado
        int index = atomicAdd(&offsets[key], 1);
        iorder[index] = i;
    }
);
*/
    //BEGIN Paralelizar orden
    /*
        thrust::device_vector<int> d_ivect_size_T(n, 0);
        thrust::device_vector<int> d_iorder_T(n, 0);
        ...
        */
        ...
        /*
        //terminar con una reducción por clave:
